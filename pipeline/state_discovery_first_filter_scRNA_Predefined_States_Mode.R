...
input_dir = file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", states_output_folder, cell_type)
output_dir = file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", states_output_folder, cell_type)


# raw_data = read.delim(file.path(exp_dir, "expression_top_genes_scaled.txt"))
raw_data = read.delim(file.path(input_dir, "expression_top_genes_scaled_step_5P1.txt"))
...
#### NEW CODE ADDED - CHANGED FROM W TO OTHER GENERATED MATRIX
# W = read.delim(file.path(output_dir, "W.txt"))
W = read.delim(file.path(output_dir, "Average_Cell_State_Gene_Expression_Matrix_W.txt"))
...
write.table(tmp, file.path(output_dir, "mapping_to_initial_states_first_filter.txt"), sep = "\t", row.names = F)
...
write.table(H, file.path(output_dir, "state_abundances_first_filter.txt"), sep = "\t")
...
write.table(classes, file.path(output_dir, "state_assignment_first_filter.txt"), sep = "\t")

data = raw_data
data = data[,match(classes$ID, colnames(data))]

# clinical = read_clinical(colnames(data), dataset = dataset, dataset_type = "discovery")
clinical = read_clinical_manual(colnames(data), annotation_file_path = annotation_file_path, dataset = dataset)            
