...
classes = read.delim(file.path(input_dir, "initial_state_assignment.txt"))

data = raw_data
data = data[,match(classes$ID, colnames(data))]

# clinical = read_clinical(colnames(data), dataset = dataset, dataset_type = "discovery")
# clinical = read_clinical_manual(colnames(data), dataset_type = "discovery")
clinical = read_clinical_manual(colnames(data), annotation_file_path = annotation_file_path, dataset_type = "discovery")
