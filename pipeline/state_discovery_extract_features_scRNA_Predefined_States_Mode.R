...
input_folder_file_path = args[4]
output_folder_file_path = args[5]
...
annotation = read.delim(file.path(input_dir, "initial_state_assignment.txt"))

annotation = annotation[match(colnames(scaled_data), annotation$ID),]

# NEW CODE ADDED TO ACCOUNT FOR ONE STATE
# annotationStates = annotation$State
...
#     write.table(log_data, file.path(output_dir, "expression_full_matrix_log2_step_5P1.txt"), sep = "\t", row.names = T)
#     write.table(log_data, file.path(output_dir, "expression_top_genes_log2_step_5P1.txt"), sep = "\t", row.names = T)

#     # line below contains log_data instead of data
#     write.table(log_data, file.path(output_dir, "expression_top_genes_scaled_step_5P1.txt"), sep = "\t", row.names = T)
        
#     write.table(scaled_data, file.path(output_dir, "expression_full_matrix_scaled_step_5P1.txt"), sep = "\t", row.names = T)
...
#     write.table(data, file.path(output_dir, "expression_top_genes_scaled_step_5P1.txt"), sep = "\t", row.names = T)
#     write.table(scaled_data, file.path(output_dir, "expression_full_matrix_scaled_step_5P1.txt"), sep = "\t", row.names = T)
# }
