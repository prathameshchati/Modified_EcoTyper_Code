...
NMFGenerateW <- function(cellTypeBinaryH, scaledExpMat) {
...
#     dummyHF = dummyHF[,colnames(to_predict)]
    ...
    W_nodup <- W[!duplicated(rownames(W)),]

#     rownames(W) = c(paste0(rownames(scaledExpMatF), "__pos"),paste0(rownames(scaledExpMatF), "__neg"))
    rownames(W) = c(paste0(rownames(W_nodup), "__pos"),paste0(rownames(W_nodup), "__neg"))
