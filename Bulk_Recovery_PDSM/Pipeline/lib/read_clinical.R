...
read_clinical_manual <- function(sample_list, annotation_file_path, dataset = "Lung", dataset_type = "discovery", row_names = T, first_10 = F)
{
	...
	#if(!file.exists(file.path("../datasets", dataset_type, dataset, "annotation.txt")))
    if(!file.exists(file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", annotation_file_path)))
	{
		cat(paste0("No annotation file found for dataset: ", dataset, ". Creating empty annotation.\n"))
		clinical = data.frame(ID = sample_list)
		return(clinical)
	...
	# clinical = read.delim(file.path("../datasets",dataset_type, dataset, "annotation.txt"))
    clinical = read.delim(file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", annotation_file_path))
...
#     if(!file.exists(file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", "Test_8_Peng_Discovered_States_EcoTyper_PDSM_Run_083022", "TCGA_Recovery_090122", "Inputs", "annotation.txt")))
        if(!file.exists(file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", bulkInputDir, "annotation.txt")))
	{
		cat(paste0("No annotation file found for dataset: ", dataset, ". Creating empty annotation.\n"))
		clinical = data.frame(ID = sample_list)
		return(clinical)
...
#     clinical = read.delim(file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", "Test_8_Peng_Discovered_States_EcoTyper_PDSM_Run_083022", "TCGA_Recovery_090122", "Inputs", "annotation.txt"))
   clinical = read.delim(file.path("../Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", bulkInputDir, "annotation.txt"))
