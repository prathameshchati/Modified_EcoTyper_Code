...
# states_dir = file.path("/duo4/users/pchati/ecotyper/Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", "Test_8_Peng_Discovered_States_EcoTyper_PDSM_Run_083022", cell_type)
# states_dir = file.path("/duo4/users/pchati/ecotyper/Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", "Test_14_Peng_DS_Discovered_States_EcoTyper_PDSM_Run_090522", cell_type)
states_dir = file.path("/duo4/users/pchati/ecotyper/Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", keyDir, cell_type)

# output_dir = file.path("../EcoTyper", dataset, fractions, "Cell_States", "recovery", new_dataset, cell_type, n_states) 
# output_dir = file.path("/duo4/users/pchati/ecotyper/Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", "Test_8_Peng_Discovered_States_EcoTyper_PDSM_Run_083022", "TCGA_Recovery_090122", cell_type) 
output_dir = file.path("/duo4/users/pchati/ecotyper/Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", outputDir, cell_type) 

dir.create(output_dir, recursive = T, showWarning = F) 

# W = read.delim(file.path(states_dir, "W.txt"))
W = read.delim(file.path(states_dir, "Average_Cell_State_Gene_Expression_Matrix_W.txt"))
...
# data = read.delim(file.path("/duo4/users/pchati/ecotyper/Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", "Test_8_Peng_Discovered_States_EcoTyper_PDSM_Run_083022", "TCGA_Recovery_090122", "Inputs", "data.txt"))
data = read.delim(file.path("/duo4/users/pchati/ecotyper/Generate_Cell_State_Abundances_Predefined_States_Test_Outputs", inputDir, "data.txt"))
...
annotation = read_clinical_bulk(colnames(data), bulkInputDir = inputDir, dataset_type = "bulk")

#### NEW CODE ADDED
# training_gene_set_pos = gsub("__pos", "", rownames(W)[grepl("__pos", rownames(W))])
# training_gene_set_neg = gsub("__neg", "", rownames(W)[grepl("__neg", rownames(W))])

# data = data[match(training_gene_set_pos, rownames(data)),]
# rownames(data) = training_gene_set_pos

# WF = W + 0.0001
# WF = W + .Machine$double.eps
# W = W + .Machine$double.eps

####

#### TWO LINES BELOW COMMENTED OUT - WERE PRESENT INITIALLY
...
# nmf_reference <- NMFpredict(WF, data) 
...
### WORRY ABOUT HEATMAP GENERATION LATER - RELIES ON STEPS 6 AND 7 OF ECOTYPER
